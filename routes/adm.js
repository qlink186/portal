...
{
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM m_peg',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			console.log(errornya);
...
function berita_top_10(req, res, next)
{
	var session = req.session;
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM v_berita ORDER BY tgl_terbit ASC LIMIT 10 ',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			console.log(errornya);
...
router.get('/',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		res.render('adm/adm', { title: 'Dashboard', menu: 'adm', session: req.session});
     });
});

/* GET adm page. */
router.get('/adm-artikel-list',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT d_berita.*, d_berita_kategori.kategori as kategori, m_peg.gelar_depan as gelar_depan, m_peg.gelar_belakang as gelar_belakang, m_peg.nama as penulis FROM d_berita LEFT JOIN d_berita_kategori ON d_berita.id_kategori = d_berita_kategori.id_kategori LEFT JOIN d_users ON d_berita.id_user = d_users.id_user LEFT JOIN m_peg ON d_users.id_peg = m_peg.id_peg ORDER BY tgl_post DESC',function(err,rows)
		{
			...
			// console.log(rows);
			res.render('adm/adm-artikel-list',{title:'Data Artikel', artikel_list:rows, menu:'adm-artikel-list', session: req.session });
		});
	});
});

router.get('/adm-artikel-detail/(:id)',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita where id_berita='+req.params.id,function(err,rows)
		{
			if(err)
			{
				...
				res.redirect('/adm/adm-artikel-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-artikel-detail',{title:"Berita ",artikel:rows[0], menu:'adm-artikel-detail', session: req.session});
...
router.get('/adm-artikel-tulis', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita_kategori',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-artikel-tulis',{title:'Data Artikel', kategori:rows, menu:'adm-artikel-tulis', session: req.session });
		});
	});
});

router.post('/adm-artikel-tulis', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-artikel-detail/'+result.insertId);
		});
	});
});

 router.get('/adm-artikel-edit/(:id)', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita where id_berita='+req.params.id,function(err,rows)
		{
			if(err)
			...
			var query = connection.query('SELECT * FROM d_berita_kategori',function(err,rows)
			{
				...
				var query = connection.query('SELECT * FROM d_berita_kategori where id_kategori='+berita.id_kategori,function(err,rows)
				{
					...
					res.render('adm/adm-artikel-edit-editor',{title:'Data Artikel', kategori:kategori, kategori_artikel:kategori_artikel, artikel:berita, menu:'adm-artikel-edit-editor', session: req.session });
				});
			});
		});
	});
...
router.post('/adm-artikel-edit', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-artikel-delete/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-artikel-terbitkan/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
		var id_berita = req.params.id;
...
router.get('/adm-artikel-kategori',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita_kategori',function(err,rows)
		{
			if(err)
			...
			req.flash('msg_error', errornya);
			// console.log(rows);
			res.render('adm/adm-artikel-kategori',{title:'Data Kategori', kategori_list:rows, menu:'adm-artikel-kategori', session: req.session });
		});
	});
});

router.post('/adm-artikel-kategori-tulis', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
		var data = {            
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-artikel-kategori');
		});
	});
});

router.get('/adm-artikel-kategori-edit/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-artikel-editor', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-artikel-editor',{title:"Data Editor",all_pegawai:rows, menu:'adm-artikel-editor', session: req.session});
		});
...
//kepegawaian
router.get('/adm-kepegawaian-list', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-kepegawaian-list',{title:"Data Pegawai",all_pegawai:rows, menu:'adm-kepegawaian-list', session: req.session});
		});
         //console.log(query.sql);
     });
});

router.get('/adm-kepegawaian-detail/(:id)', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-kepegawaian-detail',{title:"Detail Pegawai",pegawai:rows, menu:'adm-kepegawaian-detail', session: req.session});
		});
         //console.log(query.sql);
     });
});

router.get('/adm-kepegawaian-tambah', authentication_mdl.is_login, opd_data, status_peg_data, gol_ruang_data, eselon_data, jabatan_data, agama_data, pendidikan_data, pendidikan_umum_data, kel_data, kec_data, kotakab_data, function(req, res, next) {
	...
	res.render('adm/adm-kepegawaian-tambah',{ title: 'Tambah Pegawai', menu: 'adm-kepegawaian-tambah', session: req.session, opd:opd_rows, status_peg:status_peg_rows, gol_ruang:gol_ruang_rows, eselon:eselon_rows, jabatan:jabatan_rows, agama:agama_rows, pendidikan:pendidikan_rows, pendidikan_umum:pendidikan_umum_rows, kel:kel_rows, kec:kec_rows, kotakab:kotakab_rows});	
});

router.post('/adm-kepegawaian-tambah', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;	
		var data = input;
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-kepegawaian-detail/'+result.insertId);
		});
	});
});

router.get('/adm-kepegawaian-edit/(:id)', authentication_mdl.is_login, opd_data, status_peg_data, gol_ruang_data, eselon_data, jabatan_data, agama_data, pendidikan_data, pendidikan_umum_data, kel_data, kec_data, kotakab_data, function(req, res, next) {
	var opd_rows = req.opd;
	var status_peg_rows = req.status_peg;
	var gol_ruang_rows = req.gol_ruang;
	var eselon_rows = req.eselon;
	...
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT m_peg.*, m_agama.agama as nama_agama, m_kel.kel as nama_kel, m_kec.kec as nama_kec, m_kotakab.kotakab as nama_kotakab, m_status_peg.ket_peg as nama_ket_peg, m_gol.gol as gol, m_gol.ruang as ruang, m_eselon.eselon as nama_eselon, m_jabatan.jabatan as nama_jabatan, m_pend.tk_pend as tk_pend, m_pend.ket_pend as ket_pend, m_pend_umum.pendidikan_umum as pendidikan_umum, m_opd.nama_opd as nama_opd FROM m_peg LEFT JOIN m_agama ON m_agama.id_agama = m_peg.agama LEFT JOIN m_kel ON m_kel.id_kel = m_peg.alamat_kel LEFT JOIN m_kec ON m_kec.id_kec = m_peg.alamat_kec LEFT JOIN m_kotakab ON m_kotakab.id_kotakab = m_peg.alamat_kota LEFT JOIN m_status_peg ON m_status_peg.id_status_peg = m_peg.status_peg LEFT JOIN m_gol ON m_gol.id_gol = m_peg.gol_ruang LEFT JOIN m_eselon ON m_eselon.id_eselon = m_peg.eselon LEFT JOIN m_jabatan ON m_jabatan.id_jabatan = m_peg.jabatan LEFT JOIN m_pend ON m_pend.id_pend = m_peg.pend_terakhir LEFT JOIN m_pend_umum ON m_pend_umum.id_pend_umum = m_peg.pend_umum LEFT JOIN m_opd ON m_opd.id_opd = m_peg.id_opd WHERE id_peg='+req.params.id,function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-kepegawaian-edit',{ title: 'Edit Pegawai', menu: 'adm-kepegawaian-edit', peg:rows[0], session: req.session, opd:opd_rows, status_peg:status_peg_rows, gol_ruang:gol_ruang_rows, eselon:eselon_rows, jabatan:jabatan_rows, agama:agama_rows, pendidikan:pendidikan_rows, pendidikan_umum:pendidikan_umum_rows, kel:kel_rows, kec:kec_rows, kotakab:kotakab_rows});
		});
	});
});

router.post('/adm-kepegawaian-edit', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-kepegawaian-delete/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
		...
		function(err, result) {			
			...
			console.log(err);
			if (result)
				res.redirect('/adm/adm-kepegawaian-list/');
		});
	});
});

//siaran pers
router.get('/adm-siaranpers-list',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT d_siaranpers.*, m_peg.gelar_depan as gelar_depan, m_peg.gelar_belakang as gelar_belakang, m_peg.nama as penulis FROM d_siaranpers LEFT JOIN d_users ON d_siaranpers.id_user = d_users.id_user LEFT JOIN m_peg ON d_users.id_peg = m_peg.id_peg ORDER BY tgl_post DESC',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-siaranpers-list',{title:'Data Siaran Pers', siaranpers:rows, menu:'adm-siaranpers-list', session: req.session });
		});
	});
});

router.get('/adm-siaranpers-detail/(:id)',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_siaranpers where id_siaranpers='+req.params.id,function(err,rows)
		{
			if(err)
			{
				var errornya  = ("Error Selecting : %s ",err );
				req.flash('msg_error', errors_detail);
				res.redirect('/adm/adm-siaranpers-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-siaranpers-detail',{title:"Siaran Pers ",siaranpers:rows[0], menu:'adm-siaranpers-detail', session: req.session});
...
router.get('/adm-siaranpers-tambah', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_siaranpers',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-siaranpers-tambah',{title:'Data Siaran Pers', menu:'adm-siaranpers-tambah', session: req.session });
		});
	});
});

router.post('/adm-siaranpers-tambah', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
		var img = '';
		...
		var query = connection.query('INSERT INTO d_siaranpers SET ?', data,
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-siaranpers-detail/'+result.insertId);
		});
	});
});

router.get('/adm-siaranpers-edit/(:id)', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_siaranpers where id_siaranpers='+req.params.id,function(err,rows)
		{
			if(err)
			...
			res.render('adm/adm-siaranpers-edit',{title:'Data Siaran Pers', siaranpers:siaranpers, menu:'adm-siaranpers-edit', session: req.session });
		});
	});
});

router.post('/adm-siaranpers-edit', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-siaranpers-delete/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-siaranpers-terbitkan/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
//kalender event
router.get('/adm-kalenderevent-list',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT d_kalenderevent.*, m_peg.gelar_depan as gelar_depan, m_peg.gelar_belakang as gelar_belakang, m_peg.nama as penulis FROM d_kalenderevent LEFT JOIN d_users ON d_kalenderevent.id_user = d_users.id_user LEFT JOIN m_peg ON d_users.id_peg = m_peg.id_peg ORDER BY tgl_post DESC',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-kalenderevent-list',{title:'Data Kalender Event', kalenderevent:rows, menu:'adm-kalenderevent-list', session: req.session });
		});
	});
});

router.get('/adm-kalenderevent-detail/(:id)',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_kalenderevent where id_kalenderevent='+req.params.id,function(err,rows)
		{
			if(err)
			{
				var errornya  = ("Error Selecting : %s ",err );
				req.flash('msg_error', errors_detail);
				res.redirect('/adm/adm-kalenderevent-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-kalenderevent-detail',{title:"Kalender Event",kalenderevent:rows[0], menu:'adm-kalenderevent-detail', session: req.session});
				}
			}
		});
	});
...
router.get('/adm-kalenderevent-tambah', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_kalenderevent',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-kalenderevent-tambah',{title:'Data Kalender Event', menu:'adm-kalenderevent-tambah', session: req.session });
		});
	});
});

router.post('/adm-kalenderevent-tambah', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;	
		var img = '';
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-kalenderevent-detail/'+result.insertId);
		});
	});
});

router.get('/adm-kalenderevent-edit/(:id)', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_kalenderevent where id_kalenderevent='+req.params.id,function(err,rows)
		{
			if(err)
			...
			res.render('adm/adm-kalenderevent-edit',{title:'Data Kalender Event', kalenderevent:kalenderevent, menu:'adm-kalenderevent-edit', session: req.session });
		});
	});
});

router.post('/adm-kalenderevent-edit', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
router.get('/adm-kalenderevent-delete/(:id)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
...
//MODUL GALLERY ALBUM
router.get('/adm-gallery-album',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_gallery_album ORDER BY tgl_post DESC',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-gallery-album',{title:'Data Gallery', gallery_album:rows, menu:'adm-gallery-album', session: req.session });
		});
	});
});

router.get('/adm-gallery-list/(:id)',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT d_gallery.*, d_gallery_album.nama as nama_album FROM d_gallery LEFT JOIN d_gallery_album ON d_gallery_album.id_gallery_album = d_gallery.id_gallery_album WHERE d_gallery.id_gallery_album='+req.params.id+' ORDER BY d_gallery.tgl_post DESC',function(err,rows)
		{
			...
			req.flash('msg_error', errornya);
			console.log(rows);
			res.render('adm/adm-gallery-list',{title:'Data Gallery', gallery:rows, menu:'adm-gallery-list', session: req.session });
		});
	});
});

router.get('/adm-gallery-detail/(:id)',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM gallery where id_gallery='+req.params.id,function(err,rows)
		{
			if(err)
			{
				var errornya  = ("Error Selecting : %s ",err );
				req.flash('msg_error', errors_detail);
				res.redirect('/adm/adm-gallery-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-gallery-detail',{title:"Gallery",gallery:rows[0], menu:'adm-gallery-detail', session: req.session});
				}
			}
		});
	});
...
router.get('/adm-gallery-tambah', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_gallery',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-gallery-tambah',{title:'Data Gallery', menu:'adm-gallery-tambah', session: req.session });
		});
	});
});

router.post('/adm-gallery-tambah', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
		var data = {            
		...
		function(err, result) {		
			...
			if (result)
				...
				res.redirect('/adm/adm-gallery-list/'+id_gallery_album);
		});		
	});
});

router.get('/adm-gallery-delete/(:id)/(:id2)', authentication_mdl.is_login, function(req,res,next){
	var input = JSON.parse(JSON.stringify(req.body));
	req.getConnection(function(err,connection){
		var session = req.session.user;
		...
		var query = connection.query('DELETE FROM d_gallery WHERE id_gallery='+id_gallery, function(err, result) {			
			...
			console.log(err);
			if (result)
				res.redirect('/adm/adm-gallery-list/'+req.params.id2);
		});
	});
});

router.get('/adm-email-baca', function(req, res, next) {
  res.render('adm/adm-email-baca', { title: 'E-Mail Masuk', menu: 'adm-email-baca' });
});

router.get('/adm-email-draft', function(req, res, next) {
  res.render('adm/adm-email-draft', { title: 'Draft', menu: 'adm-email-draft' });
});

router.get('/adm-email-inbox', function(req, res, next) {
  res.render('adm/adm-email-inbox', { title: 'Inbox', menu: 'adm-email-inbox' });
});

router.get('/adm-email-junk', function(req, res, next) {
  res.render('adm/adm-email-junk', { title: 'Junk', menu: 'adm-email-junk' });
});

router.get('/adm-email-kotakkeluar', function(req, res, next) {
  res.render('adm/adm-email-kotakkeluar', { title: 'Kotak Keluar', menu: 'adm-email-kotakkeluar' });
});

router.get('/adm-email-tulis', function(req, res, next) {
  res.render('adm/adm-email-tulis', { title: 'Tulis E-Mail', menu: 'adm-email-tulis' });
});

router.get('/adm-email-terhapus', function(req, res, next) {
  res.render('adm/adm-email-terhapus', { title: 'Item Terhapus', menu: 'adm-email-terhapus' });
});

router.get('/adm-email-terkirim', function(req, res, next) {
  res.render('adm/adm-email-terkirim', { title: 'Item Terkirim', menu: 'adm-email-terkirim' });
});

router.get('/adm-profil-setting', authentication_mdl.is_login, user_single_data, opd_data, status_peg_data, gol_ruang_data, eselon_data, jabatan_data, agama_data, pendidikan_data, pendidikan_umum_data, kel_data, kec_data, kotakab_data, function(req, res, next) {
	var opd_rows = req.opd;
	var status_peg_rows = req.status_peg;
	var gol_ruang_rows = req.gol_ruang;
	var eselon_rows = req.eselon;
	...
	req.getConnection(function(err,connection){		
		var query = connection.query('SELECT m_peg.*, m_agama.agama as nama_agama, m_kel.kel as nama_kel, m_kec.kec as nama_kec, m_kotakab.kotakab as nama_kotakab, m_status_peg.ket_peg as nama_ket_peg, m_gol.gol as gol, m_gol.ruang as ruang, m_eselon.eselon as nama_eselon, m_jabatan.jabatan as nama_jabatan, m_pend.tk_pend as tk_pend, m_pend.ket_pend as ket_pend, m_pend_umum.pendidikan_umum as pendidikan_umum, m_opd.nama_opd as nama_opd FROM m_peg LEFT JOIN m_agama ON m_agama.id_agama = m_peg.agama LEFT JOIN m_kel ON m_kel.id_kel = m_peg.alamat_kel LEFT JOIN m_kec ON m_kec.id_kec = m_peg.alamat_kec LEFT JOIN m_kotakab ON m_kotakab.id_kotakab = m_peg.alamat_kota LEFT JOIN m_status_peg ON m_status_peg.id_status_peg = m_peg.status_peg LEFT JOIN m_gol ON m_gol.id_gol = m_peg.gol_ruang LEFT JOIN m_eselon ON m_eselon.id_eselon = m_peg.eselon LEFT JOIN m_jabatan ON m_jabatan.id_jabatan = m_peg.jabatan LEFT JOIN m_pend ON m_pend.id_pend = m_peg.pend_terakhir LEFT JOIN m_pend_umum ON m_pend_umum.id_pend_umum = m_peg.pend_umum LEFT JOIN m_opd ON m_opd.id_opd = m_peg.id_opd WHERE m_peg.id_peg='+user_row.id_peg,function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-profil-setting',{ title: 'Pengaturan Profil', menu: 'adm-profil-setting', member:rows[0], session: req.session, opd:opd_rows, status_peg:status_peg_rows, gol_ruang:gol_ruang_rows, eselon:eselon_rows, jabatan:jabatan_rows, agama:agama_rows, pendidikan:pendidikan_rows, pendidikan_umum:pendidikan_umum_rows, kel:kel_rows, kec:kec_rows, kotakab:kotakab_rows});
		});
	});
  	// res.render('adm/adm-profil-setting', { title: 'Pengaturan Profil', menu: 'adm-profil-setting', session: req.session });
});
...
router.post('/adm-profile-edit', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {			
			...
			console.log(err);
			if (result)
				res.redirect('/adm/adm-profil-setting/');
		});
	});
});
	...
	req.getConnection(function(err,connection){		
		var query = connection.query('SELECT * FROM m_peg WHERE id_peg='+req.params.id,function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-profile-picture',{ title: 'Pengaturan Profil', menu: 'adm-profile-picture', member:rows[0], session: req.session});
		});
	});
  	// res.render('adm/adm-profil-setting', { title: 'Pengaturan Profil', menu: 'adm-profil-setting', session: req.session });
});
	...
	req.getConnection(function(err,connection){		
		var query = connection.query('SELECT * FROM m_peg WHERE id_peg='+req.params.id,function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-profile-akun',{ title: 'Pengaturan Profil', menu: 'adm-profile-picture', member:rows[0], session: req.session, user:req.user});
		});
	});
  	// res.render('adm/adm-profil-setting', { title: 'Pengaturan Profil', menu: 'adm-profil-setting', session: req.session });
});
...
router.get('/adm-download-list', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-download-list',{title:"Data Download",all_download:rows, menu:'adm-download-list', session: req.session});
		});
         //console.log(query.sql);
     });
});
...
router.get('/adm-download-detail/(:id)', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-download-detail',{title:"Data Download",download:rows, menu:'adm-download-detail', session: req.session});
		});
         //console.log(query.sql);
     });
});
...
router.get('/adm-download-tambah', authentication_mdl.is_login, opd_data, jenis_file_data, kategori_file_data, function(req, res, next) {
	...
	res.render('adm/adm-download-tambah',{ title: 'Tambah Download', menu: 'adm-download-tambah', session: req.session, opd:opd_rows, jenis_file:jenis_file_rows, kategori_file:kategori_file_rows});	
});
...
router.post('/adm-download-tambah', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-download-list/');
		});
	});
});
	...
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT d_download.*, m_opd.nama_opd as nama_opd, m_dip_jenis_file.jenis_file as nama_jenis_file, m_dip_kategori.kategori_dip as kategori_dip FROM d_download LEFT JOIN m_opd ON d_download.id_opd = m_opd.id_opd LEFT JOIN m_dip_jenis_file ON d_download.jenis_file = m_dip_jenis_file.id_jenis_file LEFT JOIN m_dip_kategori ON d_download.kategori_file = m_dip_kategori.id_kategori_dip where d_download.id_download='+req.params.id,function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-download-edit',{ title: 'Edit Download', menu: 'adm-download-edit', download:rows[0], session: req.session, opd:opd_rows, jenis_file:jenis_file_rows, kategori_file:kategori_file_rows});
		});
	});
});
...
router.get('/adm-download-kategori',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM m_dip_jenis_file',function(err,rows)
		{
			...
			req.flash('msg_error', errornya);
			// console.log(rows);
			res.render('adm/adm-download-kategori',{title:'Data Download Kategori', kategori_download_list:rows, menu:'adm-download-kategori', session: req.session });
		});
	});
});
...
router.post('/adm-download-kategori-tulis', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-download-kategori/');
		});
	});
});
...
router.get('/adm-member-list', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-member-list',{title:"Data Member",all_member:rows, menu:'adm-member-list', session: req.session});
		});
         //console.log(query.sql);
     });
});
	...
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM v_users_active where id_user='+req.params.id,function(err,rows)
		{
			if(err)
			{
				var errornya  = ("Error Selecting : %s ",err );
				req.flash('msg_error', errors_detail);
				res.redirect('/adm/adm-member-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-member-detail',{title:"Member",member:rows[0], menu:'adm-member-detail', session: req.session});
				}
			}
		});
	});
...
router.get('/adm-member-tambah', authentication_mdl.is_login, peg_data, hakakses_data, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_users',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-member-tambah',{title:'Data Member', menu:'adm-member-tambah', session: req.session, peg:req.peg, hakakses:req.hakakses });
		});
	});
});
...
router.post('/adm-member-tambah', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-member-detail/'+result.insertId);
		});
	});
});
...
router.get('/adm-member-edit/(:id)', authentication_mdl.is_login, peg_data, hakakses_data, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		var query = connection.query('SELECT d_users.*, m_hakakses.hakakses as hakakses, m_peg.nama as nama FROM d_users LEFT JOIN m_hakakses ON d_users.id_hakakses = m_hakakses.id_hakaskses LEFT JOIN m_peg ON d_users.id_peg = m_peg.id_peg where d_users.username='+req.params.id, function(err,rows)
		{
			...
			console.log(rows);
			var member = rows[0];
			res.render('adm/adm-member-edit',{title:'Member Event', member:member, menu:'adm-member-edit', session: req.session, peg:req.peg, hakakses:req.hakakses});
		});
	});
});
...
router.get('/adm-halaman-list', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_halaman',function(err,rows)
		{
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-halaman-list',{title:"Halaman",halaman_profil:rows, menu:'adm-halaman-list', session: req.session});
		});
         //console.log(query.sql);
     });
});
...
router.get('/adm-halaman-detail/(:id)',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_halaman where id_halaman='+req.params.id,function(err,rows)
		{
			if(err)
			{
				var errornya  = ("Error Selecting : %s ",err );
				req.flash('msg_error', errors_detail);
				res.redirect('/adm/adm-halaman-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-halaman-detail',{title:"Halaman ",halaman_profil:rows[0], menu:'adm-halaman-detail', session: req.session});
	...
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_halaman',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-halaman-tambah',{title:'Data Halaman', menu:'adm-halaman-tambah', session: req.session });
		});
	});
});
...
router.post('/adm-halaman-tambah', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		var query = connection.query('INSERT INTO d_halaman SET ?', data,
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-halaman-detail/'+result.insertId);
		});
	});
});
...
router.get('/adm-halaman-edit/(:id)', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_halaman where id_halaman='+req.params.id,function(err,rows)
		{
			...
			res.render('adm/adm-halaman-edit',{title:'Data Halaman', halaman_profil:halaman_profil, menu:'adm-halaman-edit', session: req.session });
		});
	});
});
...
router.post('/adm-halaman-edit', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		var query = connection.query('UPDATE d_halaman SET ? WHERE id_halaman = '+id_halaman, data,
		function(err, result) {			
			console.log(result);
			console.log(err);
			if (result)
...
router.get('/adm-halaman-delete/(:id)', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		var query = connection.query('DELETE FROM d_halaman WHERE id_halaman = '+id_halaman,
		function(err, result) {			
			console.log(result);
			console.log(err);
			if (result)
...
router.get('/adm-slideshow-list',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT d_berita.*, d_berita_kategori.kategori as kategori, m_peg.gelar_depan as gelar_depan, m_peg.gelar_belakang as gelar_belakang, m_peg.nama as penulis FROM d_berita LEFT JOIN d_berita_kategori ON d_berita.id_kategori = d_berita_kategori.id_kategori LEFT JOIN d_users ON d_berita.id_user = d_users.id_user LEFT JOIN m_peg ON d_users.id_peg = m_peg.id_peg ORDER BY tgl_post DESC',function(err,rows)
		{
			...
			req.flash('msg_error', errornya);
			// console.log(rows);
			res.render('adm/adm-slideshow-list',{title:'Data Artikel', artikel_list:rows, menu:'adm-slideshow-list', session: req.session });
		});
	});
});
	...
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita where id_berita='+req.params.id,function(err,rows)
		{
			if(err)
			{
				var errornya  = ("Error Selecting : %s ",err );
				req.flash('msg_error', errors_detail);
				res.redirect('/adm/adm-slideshow-list');
			}else
			{
				if(rows.length <=0)
				{
				...
				{	
					console.log(rows);
					res.render('adm/adm-slideshow-detail',{title:"Berita ",artikel:rows[0], menu:'adm-slideshow-detail', session: req.session});
	...
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita_kategori',function(err,rows)
		{
			if(err)
				var errornya  = ("Error Memilih : %s ",err );
			req.flash('msg_error', errornya);
			res.render('adm/adm-slideshow-tulis',{title:'Data Artikel', kategori:rows, menu:'adm-slideshow-tulis', session: req.session });
		});
	});
});
...
router.post('/adm-slideshow-tulis', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-slideshow-detail/'+result.insertId);
		});
	});
});
		...
		var query = connection.query('SELECT * FROM d_berita where id_berita='+req.params.id,function(err,rows)
		{
			...
			var query = connection.query('SELECT * FROM d_berita_kategori',function(err,rows)
			{
				...
				var query = connection.query('SELECT * FROM d_berita_kategori where id_kategori='+berita.id_kategori,function(err,rows)
				{
					...
					req.flash('msg_error', errornya);
					var kategori_artikel = rows[0];
					res.render('adm/adm-slideshow-edit',{title:'Data Artikel', kategori:kategori, kategori_artikel:kategori_artikel, artikel:berita, menu:'adm-slideshow-edit', session: req.session });
				});
			});
		});
	});
...
router.get('/adm-slideshow-kategori',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM d_berita_kategori',function(err,rows)
		{
			...
			req.flash('msg_error', errornya);
			// console.log(rows);
			res.render('adm/adm-slideshow-kategori',{title:'Data Kategori', kategori_list:rows, menu:'adm-artikel-kategori', session: req.session });
		});
	});
});
...
router.post('/adm-slideshow-kategori-tulis', authentication_mdl.is_login, function(req,res,next){
	...
	req.getConnection(function(err,connection){
		...
		function(err, result) {		
			console.log(err);
			if (result)
				res.redirect('/adm/adm-slideshow-kategori/');
		});
	});
});
...
router.get('/adm-slideshow-editor', authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		...
		if(err)
				var errornya  = ("Error Memilih : %s ",err );
				req.flash('msg_error', errornya);
				res.render('adm/adm-slideshow-editor',{title:"Data Editor",all_pegawai:rows, menu:'adm-slideshow-editor', session: req.session});
		});
         //console.log(query.sql);
     });
});
